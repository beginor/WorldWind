...
namespace WorldWind {
	...
	public class BinaryTriangleTree {
		private CustomVertex.PositionTextured[] _elevatedVertices;
		private int _vertexDensity;
		private int _verticeDensity;
		private int _margin;
		private double _layerRadius;
		private int _maxVariance;
		private BinaryTriangle[] _treeList;
		private int _treeListLength;
		private int _terrainFaceCount;
		private short nw, sw, ne, se;
		...
		private void BuildFace(short f) {
			if (this._treeList[f].lc
			    != -1) // face already has childs
			{
				BuildFace(this._treeList[f].lc);
				BuildFace(this._treeList[f].rc);
			}
			else {
				if (FaceVariance(this._treeList[f].i1, this._treeList[f].i2, this._treeList[f].i3)
				    > this._maxVariance) {
		...
		private void SplitFace(short f) {
			if (this._treeList[f].bn
			    != -1) // Check bottom neighbour first
			{
		...
		private int FaceCount() {
			...
			for (short i = 0; i < this._treeListLength; i++) {
				if (this._treeList[i].rc
				    == -1) {
					tot++; // no child is to be drawn
				}
			}
			return tot;
		}
