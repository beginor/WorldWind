...
namespace WorldWind.Camera {
	...
	public class CameraBase {
		...
		protected static readonly Angle maxTilt = Angle.FromDegrees(90.0);
		...
		private static Point3d cameraUpVector = new Point3d(0, 0, 1);
		public Point3d ReferenceCenter = new Point3d(0, 0, 0);

		// Camera Reset variables
		private static int lastResetTime; // Used by Reset() to keep track type of reset.
		...
		public bool IsPointGoto {
			get {
				return World.Settings.cameraIsPointGoto;
			}
			set {
				World.Settings.cameraIsPointGoto = value;
			}
		}
		...
		public virtual double AltitudeAboveTerrain {
			get {
				return this._altitude - _terrainElevation;
			}
		}
		...
		public virtual double WorldRadius {
			...
			set {
				this._worldRadius = value;
			}
		}

		public Vector3 EyeDiff = new Vector3();

		private float curCameraElevation = 0;
		private float targetCameraElevation = 0;
		...
		public virtual void ComputeAbsoluteMatrices() {
			...
			m_absoluteViewMatrix = Matrix.LookAtRH(MathEngine.SphericalToCartesian(_latitude.Degrees, _longitude.Degrees, WorldRadius), Vector3.Empty, new Vector3(0, 0, 1));

			m_absoluteViewMatrix *= Matrix.RotationYawPitchRoll(0, (float) -_tilt.Radians, (float) this._heading.Radians);
		...
		public virtual void Update(Device device) {
			...
			ViewFrustum.Update(Matrix.Multiply(m_absoluteWorldMatrix, Matrix.Multiply(m_absoluteViewMatrix, m_absoluteProjectionMatrix)));
		...
		public virtual void Reset() {
			...
			if (curTime - lastResetTime < DoubleTapDelay) {
				...
				this.SetPosition(double.NaN, double.NaN, 0, maximumAltitude, 0, 0);
			}
			else {
		...
		public virtual void SetPosition(double lat, double lon, double heading, double altitude, double tilt) {
			SetPosition(lat, lon, heading, altitude, tilt, 0);
		}
		...
		public override string ToString() {
			string res = string.Format(CultureInfo.InvariantCulture, "Altitude: {6:f0}m\nView Range: {0}\nHeading: {1}\nTilt: {2}\nFOV: {7}\nPosition: ({3}, {4} @ {5:f0}m)", ViewRange, _heading, _tilt, _latitude, _longitude, _distance, _altitude, _fov);
			return res;
		}
		...
		public static Angle[] getViewBoundingBox() {
			...
			if (DrawArgs.Camera != null) {
				...
				bbox[0] = Angle.FromDegrees(-180.0);
				bbox[1] = Angle.FromDegrees(-90.0);
				bbox[2] = Angle.FromDegrees(180.0);
				bbox[3] = Angle.FromDegrees(90.0);
			}
			return bbox;
		}

		#region by zzm
		protected static double minLatitude = 19.0; // -91.0;
		protected static double maxLatitude = 31.0; // 91.0;
		protected static double minLongitude = 109.0; // -181.0;
		protected static double maxLongitude = 121.0; //181.0;

		/// <summary>
		/// 检查目的地的经度和纬度，看看有没有超出要锁定的区域
		/// </summary>
		/// <param name="latitude"></param>
		/// <param name="longitude"></param>
		//protected static void CheckLatLon(ref double latitude, ref double longitude) {
		//   // check for latitude;
		...
		//protected static void CheckLatLon(ref Angle lat, ref Angle lon) {
		//   double latitude = lat.Degrees;
		//   double longitude = lon.Degrees;
		//   CheckLatLon(ref latitude, ref longitude);
