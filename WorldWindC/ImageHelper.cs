	...
	public sealed class ImageHelper {
		...
		public static Texture LoadTexture(string textureFileName, int colorKey) {
			try {
				...
				throw new InvalidDataException(string.Format("Error reading image file '{0}'.", textureFileName));
			}
		}
		...
		public static void CreateAlphaPngFromBrightness(string srcFilePath, string destinationPngFilePath) {
			...
			BitmapData srcInfo = image.LockBits(new Rectangle(0, 0, image.Width, image.Height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
			...
			BitmapData dstInfo = transparentImage.LockBits(new Rectangle(0, 0, transparentImage.Width, transparentImage.Height), ImageLockMode.WriteOnly, PixelFormat.Format32bppArgb);
			...
			transparentImage.Save(destinationPngFilePath, ImageFormat.Png);
			image.Dispose();
		...
		public static Texture LoadTexture(string textureFileName, int transparentRangeDarkColor, int transparentRangeBrightColor) {
			...
			return new Texture(DrawArgs.StaticDevice, transparentImage, Usage.None, Pool.Managed);
		}
		...
		public static Texture LoadTexture(Stream textureStream, int colorKey) {
			try {
				Texture texture = TextureLoader.FromStream(DrawArgs.StaticDevice, textureStream, 0, 0, 1, Usage.None, World.Settings.TextureFormat, Pool.Managed, Filter.Box, Filter.Box, colorKey);
			...
			try {
				...
				using (Bitmap image = (Bitmap) Image.FromStream(textureStream)) {
					Texture texture = new Texture(DrawArgs.StaticDevice, image, Usage.None, Pool.Managed);
					return texture;
				}
			}
			catch {
				throw new InvalidDataException("Error reading image stream.");
			}
		}
		...
		public static Image LoadImage(string bitmapFileName) {
			try {
				...
				Log.Write("IMAG", "Error loading image '" + bitmapFileName + "'.");
		...
		public static Cursor LoadCursor(string relativePath) {
			...
			try {
				...
				Log.Write("IMAG", "Unable to load cursor '" + relativePath + "': " + caught.Message);
		...
		public static Texture LoadIconTexture(string relativePath) {
			try {
				...
				if (File.Exists(fullPath)) {
					return TextureLoader.FromFile(DrawArgs.StaticDevice, fullPath, 0, 0, 1, Usage.None, Format.Dxt5, Pool.Managed, Filter.Box, Filter.Box, 0);
				}
			}
			catch {
				Log.Write("IMAG", "Error loading texture '" + relativePath + "'.");
			}

			// Make a replacement warning texture with a red cross over.
			using (Bitmap bitmap = CreateDefaultImage()) {
				return new Texture(DrawArgs.StaticDevice, bitmap, 0, Pool.Managed);
			}
		}
		...
		public static void ConvertToDds(string originalImagePath, string outputDdsPath, Format format, bool eraseOriginal) {
			try {
				using (Texture t = TextureLoader.FromFile(DrawArgs.StaticDevice, originalImagePath, 0, 0, 1, 0, format, Pool.Scratch, Filter.Box | Filter.DitherDiffusion, Filter.None, 0)) {
		...
		public static void ConvertToDds(Stream originalImageStream, string outputDdsPath, Format format) {
			try {
				...
				using (Texture t = TextureLoader.FromStream(DrawArgs.StaticDevice, originalImageStream, 0, 0, 1, 0, format, Pool.Scratch, Filter.Box | Filter.DitherDiffusion, Filter.None, 0)) {
					TextureLoader.Save(outputDdsPath, ImageFileFormat.Dds, t);
				}
			}
			catch (InvalidDataException) {
