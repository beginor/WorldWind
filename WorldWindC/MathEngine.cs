...
namespace WorldWind {
	...
	public sealed class MathEngine {
		...
		public static Vector3 SphericalToCartesian(double latitude, double longitude, double radius) {
			...
			return new Vector3((float) (radCosLat*Math.Cos(longitude)), (float) (radCosLat*Math.Sin(longitude)), (float) (radius*Math.Sin(latitude)));
		}
		...
		public static Vector3 SphericalToCartesian(Angle latitude, Angle longitude, double radius) {
			...
			return new Vector3((float) (radCosLat*Math.Cos(lonRadians)), (float) (radCosLat*Math.Sin(lonRadians)), (float) (radius*Math.Sin(latRadians)));
		}
		...
		public static Point3d SphericalToCartesianD(Angle latitude, Angle longitude, double radius) {
			...
			return new Point3d(radCosLat*Math.Cos(lonRadians), radCosLat*Math.Sin(lonRadians), radius*Math.Sin(latRadians));
		}
		...
		public static double SphericalDistanceDegrees(double latA, double lonA, double latB, double lonB) {
			double radLatA = DegreesToRadians(latA);
			double radLatB = DegreesToRadians(latB);
			double radLonA = DegreesToRadians(lonA);
			double radLonB = DegreesToRadians(lonB);

			return RadiansToDegrees(Math.Acos(Math.Cos(radLatA)*Math.Cos(radLatB)*Math.Cos(radLonA - radLonB) + Math.Sin(radLatA)*Math.Sin(radLatB)));
		}
		...
		public static Angle SphericalDistance(Angle latA, Angle lonA, Angle latB, Angle lonB) {
			...
			return Angle.FromRadians(Math.Acos(Math.Cos(radLatA)*Math.Cos(radLatB)*Math.Cos(radLonA - radLonB) + Math.Sin(radLatA)*Math.Sin(radLatB)));
		}
		...
		public static Angle Azimuth(Angle latA, Angle lonA, Angle latB, Angle lonB) {
			...
			Angle tcA = Angle.FromRadians(Math.Atan2(Math.Sin(lonA.Radians - lonB.Radians)*cosLatB, Math.Cos(latA.Radians)*Math.Sin(latB.Radians) - Math.Sin(latA.Radians)*cosLatB*Math.Cos(lonA.Radians - lonB.Radians)));
			if (tcA.Radians < 0) {
		...
		public static int GetRowFromLatitude(double latitude, double tileSize) {
			return (int) Math.Round((Math.Abs(-90.0 - latitude)%180)/tileSize, 1);
		}
		...
		public static int GetColFromLongitude(double longitude, double tileSize) {
			return (int) Math.Round((Math.Abs(-180.0 - longitude)%360)/tileSize, 1);
		}
