	...
	public class WebDownloadRequest : DownloadRequest {
		...
		public string SaveFilePath {
			get {
				...
				return download.SavedFilePath.Substring(0, download.SavedFilePath.Length - TemporaryExtension.Length);
			}
			set {
		...
		public override void Start() {
			...
			if (download.SavedFilePath != null
			    && download.SavedFilePath.Length > 0) {
		...
		protected virtual void InternalDownloadComplete(WebDownload download) {
			try {
				...
				Log.Write("QUEU", download.Url + ": " + caught.Message);
			}
