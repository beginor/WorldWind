...
namespace WorldWind {
	...
	public class NltImageStore : ImageStore {
		...
		public NltImageStore(string dataSetName, string serverUri) {
			m_serverUri = serverUri;
			m_dataSetName = dataSetName;
		}

		protected override string GetDownloadUrl(QuadTile qt) {
			return string.Format(CultureInfo.InvariantCulture, "{0}?T={1}&L={2}&X={3}&Y={4}", m_serverUri, m_dataSetName, qt.Level, qt.Col, qt.Row);
		}
	}
}