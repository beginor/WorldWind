...
namespace WorldWind.Renderable {
	...
	public class TiledPlacenameSet : RenderableObject {
		...
		public TiledPlacenameSet(string name, World parentWorld, double altitude, double maximumDisplayAltitude, double minimumDisplayAltitude, string placenameListFilePath, FontDescription fontDescription, Color color, string iconFilePath) : base(name, parentWorld.Position, Quaternion.RotationYawPitchRoll(0, 0, 0)) {
			...
			m_color = System.Drawing.Color.FromArgb(150, color).ToArgb();
	...
	public class WorldWindPlacenameList {
				...
				using (BinaryWriter bw = new BinaryWriter(fs, Encoding.UTF8)) {
					...
					foreach (WorldWindPlacenameFile pf in m_placenameFiles) {
						...
						using (FileStream placenameFileStream = File.Open(dataFilePath, FileMode.Create)) {
							using (BinaryWriter placenameFileWriter = new BinaryWriter(placenameFileStream, Encoding.UTF8)) {
		...
		public void AddPlacename(string name, float lat, float lon, Hashtable metaData) {
			...
			foreach (WorldWindPlacenameFile tempFile in m_placenameFiles) {
				if (lat < tempFile.north && lat >= tempFile.south && lon >= tempFile.west
				    && lon < tempFile.east) {
					p = tempFile;
					break;
				}
