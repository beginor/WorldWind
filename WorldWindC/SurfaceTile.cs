...
namespace WorldWind {
	public class SurfaceTile : IDisposable {
		...
		private void OnDeviceReset(object sender, EventArgs e) {
			...
			m_RenderTexture = new Texture(dev, WorldSurfaceRenderer.RenderSurfaceSize, WorldSurfaceRenderer.RenderSurfaceSize, 1, Usage.RenderTarget, Format.X8R8G8B8, Pool.Default);
		...
		public void Initialize(DrawArgs drawArgs) {
			try {
				if (m_HeightData == null) {
					if (m_Level > 2) {
						TerrainTile tt = m_ParentWorldSurfaceRenderer.ParentWorld.TerrainAccessor.GetElevationArray(m_North, m_South, m_West, m_East, (int) m_ParentWorldSurfaceRenderer.SamplesPerTile + 1);
		...
		public void UpdateRenderSurface(DrawArgs drawArgs) {
			...
			using (Surface renderSurface = m_RenderTexture.GetSurfaceLevel(0)) {
				...
				drawArgs.Device.Clear(ClearFlags.Target | ClearFlags.ZBuffer, Color.Black, 1.0f, 0);
				...
				lock (m_ParentWorldSurfaceRenderer.SurfaceImages.SyncRoot) {
					for (int i = 0; i < m_ParentWorldSurfaceRenderer.SurfaceImages.Count; i++) {
						...
						if (currentSurfaceImage == null || currentSurfaceImage.ImageTexture == null || currentSurfaceImage.ImageTexture.Disposed || !currentSurfaceImage.Enabled || (currentSurfaceImage.North > m_North && currentSurfaceImage.South >= m_North) || (currentSurfaceImage.North <= m_South && currentSurfaceImage.South < m_South) || (currentSurfaceImage.West < m_West && currentSurfaceImage.East <= m_West)
						...
						m_RenderToTextureVertices[0].Color = Color.FromArgb(currentSurfaceImage.ParentRenderable.Opacity, 0, 0, 0).ToArgb();
						...
						m_RenderToTextureVertices[1].Color = Color.FromArgb(currentSurfaceImage.ParentRenderable.Opacity, 0, 0, 0).ToArgb();
						...
						m_RenderToTextureVertices[2].Color = Color.FromArgb(currentSurfaceImage.ParentRenderable.Opacity, 0, 0, 0).ToArgb();
						...
						m_RenderToTextureVertices[3].Color = Color.FromArgb(currentSurfaceImage.ParentRenderable.Opacity, 0, 0, 0).ToArgb();
						...
						drawArgs.Device.SamplerState[0].BorderColor = Color.FromArgb(0, 0, 0, 0);
						drawArgs.Device.SamplerState[1].BorderColor = Color.FromArgb(0, 0, 0, 0);
						...
						drawArgs.Device.DrawUserPrimitives(PrimitiveType.TriangleStrip, 2, m_RenderToTextureVertices);
		...
		private void buildTerrainMesh() {
			...
			for (int i = 0; i < thisVertexDensityElevatedPlus3; i++) {
				...
				for (int j = 0; j < thisVertexDensityElevatedPlus3; j++) {
					float height = -30000;
					if (i == 0 || j == 0 || i == thisVertexDensityElevatedPlus3 - 1
					    || j == thisVertexDensityElevatedPlus3 - 1) {
						double latitude = 0;
						double longitude = 0;
						float tu = 0.0f;
						float tv = 0.0f;
						...
						Vector3 v = MathEngine.SphericalToCartesian(Angle.FromDegrees(latitude), Angle.FromDegrees(longitude), m_ParentWorldSurfaceRenderer.ParentWorld.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
						...
						Vector3 v = MathEngine.SphericalToCartesian(Angle.FromDegrees(m_North - scaleFactor*latrange*(i - 1)), Angle.FromDegrees(m_West + scaleFactor*lonrange*(j - 1)), m_ParentWorldSurfaceRenderer.ParentWorld.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
			...
			for (int i = 0; i < thisVertexDensityElevatedPlus3; i++) {
				...
				for (int j = 0; j < thisVertexDensityElevatedPlus3; j++) {
					...
					if (i == 0 || j == 0 || i == thisVertexDensityElevatedPlus3 - 1
						...
						Vector3 v = MathEngine.SphericalToCartesian(Angle.FromDegrees(m_North - scaleFactor*latrange*(i - 1)), Angle.FromDegrees(0.5f*(m_West + m_East) + (float) scaleFactor*lonrange*(j - 1)), m_ParentWorldSurfaceRenderer.ParentWorld.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
			...
			for (int i = 0; i < thisVertexDensityElevatedPlus3; i++) {
				...
				for (int j = 0; j < thisVertexDensityElevatedPlus3; j++) {
					...
					if (i == 0 || j == 0 || i == thisVertexDensityElevatedPlus3 - 1
						...
						Vector3 v = MathEngine.SphericalToCartesian(Angle.FromDegrees(0.5*(m_North + m_South) - scaleFactor*latrange*(i - 1)), Angle.FromDegrees(m_West + (float) scaleFactor*lonrange*(j - 1)), m_ParentWorldSurfaceRenderer.ParentWorld.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
			...
			for (int i = 0; i < thisVertexDensityElevatedPlus3; i++) {
				...
				for (int j = 0; j < thisVertexDensityElevatedPlus3; j++) {
					...
					if (i == 0 || j == 0 || i == thisVertexDensityElevatedPlus3 - 1
						...
						Vector3 v = MathEngine.SphericalToCartesian(Angle.FromDegrees(0.5*(m_North + m_South) - scaleFactor*latrange*(i - 1)), Angle.FromDegrees(0.5*(m_West + m_East) + (float) scaleFactor*lonrange*(j - 1)), m_ParentWorldSurfaceRenderer.ParentWorld.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
		...
		public bool IsRenderable(DrawArgs drawArgs) {
			...
			if (!m_Initialized || drawArgs.WorldCamera.TrueViewRange/2 > Angle.FromDegrees(3.0*m_DynamicTextureSize*1.5f) || MathEngine.SphericalDistance(Angle.FromDegrees(_centerLat), Angle.FromDegrees(_centerLon), drawArgs.WorldCamera.Latitude, drawArgs.WorldCamera.Longitude) > Angle.FromDegrees(3.0*m_DynamicTextureSize*1.5f) || m_BoundingBox == null || !drawArgs.WorldCamera.ViewFrustum.Intersects(m_BoundingBox)
		...
		public void Render(DrawArgs drawArgs) {
			try {
				...
				if (m_SouthEastChild != null && m_SouthEastChild.m_Initialized
				    && m_SouthEastChild.IsRenderable(drawArgs)) {
				...
				if (m_RenderTexture == null || m_RequiresUpdate || checkSurfaceImageChange()
				    || m_FramesSinceLastUpdate++ > SurfaceTileRefreshHz) {
				...
				if (!nwRendered
				    && m_DynamicTexture.nwVerts != null) {
					drawArgs.Device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, m_DynamicTexture.nwVerts.Length, (m_NwIndices != null ? m_NwIndices.Length/3 : m_IndicesElevated.Length/3), (m_NwIndices != null ? m_NwIndices : m_IndicesElevated), true, m_DynamicTexture.nwVerts);
				}

				if (!neRendered
				    && m_DynamicTexture.neVerts != null) {
					drawArgs.Device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, m_DynamicTexture.neVerts.Length, (m_NeIndices != null ? m_NeIndices.Length/3 : m_IndicesElevated.Length/3), (m_NeIndices != null ? m_NeIndices : m_IndicesElevated), true, m_DynamicTexture.neVerts);
				}

				if (!swRendered
				    && m_DynamicTexture.swVerts != null) {
					drawArgs.Device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, m_DynamicTexture.nwVerts.Length, (m_SwIndices != null ? m_SwIndices.Length/3 : m_IndicesElevated.Length/3), (m_SwIndices != null ? m_SwIndices : m_IndicesElevated), true, m_DynamicTexture.swVerts);
				}

				if (!seRendered
				    && m_DynamicTexture.seVerts != null) {
					drawArgs.Device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, m_DynamicTexture.seVerts.Length, (m_SeIndices != null ? m_SeIndices.Length/3 : m_IndicesElevated.Length/3), (m_SeIndices != null ? m_SeIndices : m_IndicesElevated), true, m_DynamicTexture.seVerts);
				}
			}
			catch (ThreadAbortException) {}
			catch (Exception ex) {
	...
	internal class DynamicTexture : IDisposable {
