	...
	public class NltTerrainAccessor : TerrainAccessor {
		...
		public override float GetElevationAt(double latitude, double longitude, double targetSamplesPerDegree) {
			try {
				if (m_terrainTileService == null
				    || targetSamplesPerDegree < World.Settings.MinSamplesPerDegree) {
					return 0;
				}

				if (m_higherResolutionSubsets != null) {
					foreach (TerrainAccessor higherResSub in m_higherResolutionSubsets) {
						if (latitude > higherResSub.South && latitude < higherResSub.North && longitude > higherResSub.West
						    && longitude < higherResSub.East) {
		...
		public override TerrainTile GetElevationArray(double north, double south, double west, double east, int samples) {
			...
			if (m_higherResolutionSubsets != null) {
				...
				foreach (TerrainAccessor higherResSub in m_higherResolutionSubsets) {
					if (north <= higherResSub.North && south >= higherResSub.South && west >= higherResSub.West
					    && east <= higherResSub.East) {
			...
			for (int x = 0; x < samples; x++) {
				for (int y = 0; y < samples; y++) {
					...
					if (ttce == null || curLat < ttce.TerrainTile.South || curLat > ttce.TerrainTile.North || curLon < ttce.TerrainTile.West
				...
				if (m_tileCache.Count >= CacheSize) {
					...
					foreach (TerrainTileCacheEntry curEntry in m_tileCache.Values) {
						if (oldestTile == null) {
							...
							if (curEntry.LastAccess
							    < oldestTile.LastAccess) {
								oldestTile = curEntry;
							}
						}
					}
		...
		public class TerrainTileCacheEntry {
			...
			private TerrainTile m_terrainTile;
