...
namespace WorldWind.Terrain {
	...
	public class TerrainDownloadRequest : GeoSpatialDownloadRequest {
		...
		private const string ContentType7z = "application/x-7z-compressed";
		private const string ContentTypeXCompressed = "application/x-compressed";
		...
		protected void ProcessFile() {
			if (download.ContentType == ContentType7z
			    || download.ContentType == ContentTypeXCompressed) {
				// Decompress 7z
				...
				psi.Arguments = String.Format(CultureInfo.InvariantCulture, " x -y -o\"{1}\" \"{0}\"", compressedPath, tempDirectory);
				...
				if (File.Exists(base.SaveFilePath)) {
					File.Delete(base.SaveFilePath);
				}
				string destDir = Path.GetDirectoryName(base.SaveFilePath);
				if (!Directory.Exists(destDir)) {
					Directory.CreateDirectory(destDir);
				}
				File.Move(base.download.SavedFilePath, base.SaveFilePath);
			}
			#endregion
		}
