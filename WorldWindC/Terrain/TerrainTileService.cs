...
namespace WorldWind.Terrain {
	...
	public class TerrainTileService : IDisposable {
		...
		public TerrainTileService(string serverUrl, string dataSet, double levelZeroTileSizeDegrees, int samplesPerTile, string fileExtension, int numberLevels, string terrainTileDirectory, TimeSpan terrainTileRetryInterval, string dataType) {
		...
		public TerrainTile GetTerrainTile(double latitude, double longitude, double samplesPerDegree) {
			...
			tile.TerrainTileFilePath = string.Format(CultureInfo.InvariantCulture, @"{0}\{4}\{1:D4}\{1:D4}_{2:D4}.{3}", m_terrainTileDirectory, tile.Row, tile.Col, m_fileExtension, tile.TargetLevel);
	...
	public class TerrainTile : IDisposable {
		...
		public float GetElevationAt(double latitude, double longitude) {
			try {
				...
				float westElevation = ElevationData[lon_min, lat_min]*(1 - delta) + ElevationData[lon_min, lat_max]*delta;

				float eastElevation = ElevationData[lon_max, lat_min]*(1 - delta) + ElevationData[lon_max, lat_max]*delta;
