...
namespace WorldWind.Utilities {
	...
	public sealed class BindingsCheck {
		...
		private static extern int WSCEnumProtocols(IntPtr lpiProtocols, IntPtr lpProtocolBuffer, ref long lpdwBufferLength, ref int lpErrno);
		...
		private static int DetermineProtocolBindings() {
			int errorNumber = 0;
			...
			if (errorNumber != WSAENOBUFS) {
				return -1; // not what we expected, unable to determine number of bindings
			}
		...
		private static bool IsBindingsHotFixApplied() {
			// check if CLR version is 1.1.4322.946 or higher
			return Environment.Version >= new Version("1.1.4322.946");
		}
		...
		public static bool FiftyBindingsWarning() {
			if (DetermineProtocolBindings() <= 50
			    || IsBindingsHotFixApplied()) {
				return false;
			}

			DialogResult userChoice = MessageBox.Show("Your computer has a large number of protocols (over 50) installed,\n" + "which is not supported by the currently installed .NET version.\n\n" + "As a result, World Wind may not be able to download imagery.\n\n" + "Service pack 1 for the .NET framework 1.1 fixes this problem -\n" + "do you want to go to the download page now?\n\n" + "(Click No to continue launching World Wind)", "NASA World Wind: Fifty protocol bindings problem detected", MessageBoxButtons.YesNo);

			if (userChoice == DialogResult.Yes) {
				Process.Start("iexplore", "http://msdn.microsoft.com/netframework/downloads/updates/default.aspx");
				return true;
			}
			return false;
		}
