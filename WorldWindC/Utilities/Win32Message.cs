...
namespace WorldWind.Utilities {
	...
	public class Win32Message {
		...
		private const int LOAD_LIBRARY_AS_DATAFILE = 0x02;

		// WINHTTP DLL Error range
		private const int WINHTTP_ERROR_BASE = 12000;
		private const int WINHTTP_ERROR_LAST = WINHTTP_ERROR_BASE + 184;

		// NETMSG DLL Error range
		private const int NERR_BASE = 2100;
		private const int MAX_NERR = NERR_BASE + 899;
		...
		private static extern IntPtr LoadLibraryEx(string lpFileName, // file name of module
		...
		public static string GetMessage(int lastError) {
			...
			int dwFormatFlags = FORMAT_MESSAGE_ALLOCATE_BUFFER | // allocate the memory for us
			...
			foreach (DllDescriptor dllDesc in m_dllDescriptors) {
				...
				if (lastError >= dllDesc.firstMessage
				    && lastError <= dllDesc.lastMessage) {
			...
			dwBufferLength = FormatMessage(dwFormatFlags, hModule, lastError, 1024, // MAKELANGID (LANG_NEUTRAL, SUBLANG_DEFAULT)
			                               out pMessageBuffer, 0, IntPtr.Zero);
