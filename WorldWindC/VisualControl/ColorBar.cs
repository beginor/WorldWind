...
namespace WorldWind.VisualControl {
	...
	public class Colorbar : Form {
		private Image image;
		private string oldText; // Original legend text
		...
		public Colorbar(Form parent) {
			this.SetStyle(ControlStyles.AllPaintingInWmPaint | ControlStyles.Opaque | ControlStyles.ResizeRedraw | ControlStyles.DoubleBuffer | ControlStyles.UserPaint, true);
		...
		private void DownloadComplete(WebDownload downloadInfo) {
			...
			try {
				downloadInfo.Verify();
				Image = Image.FromStream(downloadInfo.ContentStream);
			}
			catch (Exception caught) {
				this.Visible = false;
				MessageBox.Show(caught.Message, "Legend image download failed.", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
			finally {
